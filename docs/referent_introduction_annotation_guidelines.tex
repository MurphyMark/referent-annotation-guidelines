...
Bridging-Contained is similar to Bridging, except that the anchor must be in the same noun phrase as the bridging referent. In the majority of cases, the anchor must be non-new, i.e. \texttt{given}, \texttt{unused-unknown}, or \texttt{bridging}, except for the cases when the bridging referent and its anchor are linked with the preposition \emph{of} (see below).
...
\begin{quote}
There was a woman. A dog \texttt{n} on a leash \texttt{b} was barking.
\end{quote}

\begin{quote}
There was a woman. The dog \texttt{n} on a leash \texttt{b} was barking.
\end{quote}
...
In the three examples above, `leash' is \texttt{b}, because \texttt{dog} is the anchor.
...
Tag ``it'' \texttt{g}, ``the car'' \texttt{g+fe}and ``the other car'' \texttt{g+fe}.
